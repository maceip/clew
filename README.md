...
work never started. `restart` watches the sessions and repos you already
...
before first tag; the name is **restart** — fitting, since the co-lead
feature is making restarts lossless (§9).

## Build & install

```bash
go build -o restart ./cmd/restart     # Go ≥ 1.22; single static binary
```
...
restart init               # register; archaeology; installs CLAUDE.md/AGENTS.md
                           #   include + Claude nudge hook + .gitignore entry
restart watch install      # launchd/systemd-user supervised watcher
restart status             # the glance: SESSIONS·DECIDED·LEARNED·OPEN·ALERTS
restart map                # intent × reality, absence highlighted
restart inbox              # human-blocking items only
restart backfill --budget 100000   # months of existing session history → journal
```
...
first available; configurable in `~/.restart/config.yaml`), and syncs the
journal branch through the repo's own remote. No server, no new credentials.

For agents with no session store: `restart wrap -- gemini …` (PTY tee).
Optional MCP surface: `restart mcp` (journal_search / journal_get /
journal_note).

## The restart moment (§9)

```bash
restart manifest                    # disposition pass → MANIFEST.md
$EDITOR .../MANIFEST.md             # human pass: mark [carry] / [drop]
restart manifest --out ./kit       # SEED.md (≤4KB) + genesis/ + journaled dispositions
cd ../successor && git init && restart init --carry ../old/kit/genesis
```
...
- Orphan branch `restart/journal` in the project's own remote; worktree
  checkout under `~/.restart/worktrees/<id>`.
...
- `restart redact <id>` is the one sanctioned rewrite: scrub-in-place, fresh
...
- Machine-local working state in SQLite (`~/.restart/state.db`); never the
  source of truth.
...
RESTART_FIDELITY=1 go test ./acceptance -run RealProvider -v   # live extraction gate
```
...
|---|---|---|
| 1 | §12.1 name before first tag | `restart` (binary, branch `restart/journal`, `.restart/`, `~/.restart/`) |
| 2 | §3.1 questions "carry who-can-answer" but §3.2's example (an intent) shows no field | added `asks: human\|any` on question entries |
| 3 | §7.1.3 intent "completion confirmation event" unnamed | `restart journal confirm <id> --done` |
| 4 | §7.1.3 "only a human confirm event sets contradicted" — verb unspecified | `restart journal confirm <id> --contradicts <other-id>` |
...
```
cmd/restart/          CLI: init watch status map inbox journal manifest
